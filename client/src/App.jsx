      ...
      <div className="login-card">
        ...
        <div className="lp-left">
          ...
          <div className="lp-illus-wrap">
            <img src={illustrationImg} alt="Grievance illustration"/>
          </div>
        </div>
            ...
            <div className="steps">
              {STEPS.map((l, i) => (
                <>
                  ...
                  {i < 2 && <div key={`line-${i}`} className="step-line" style={{background: step > i+1 ? 'var(--c-green)' : 'var(--c-line)'}}/>}
                </>
              ))}
            </div>
          )}
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                {list.map((c,i)=>(
                  <>
                    <tr key={c.id} style={{borderBottom:i<list.length-1?'1px solid #f5f6fc':'none',transition:'background .15s'}}
                      onMouseEnter={e=>e.currentTarget.style.background='#fafbff'}
                      onMouseLeave={e=>e.currentTarget.style.background=''}>
...
export default function App() {
  ...
  if (!user) {
    return <Login onLogin={setUser} onForgot={() => setPage("forgot")} />;
  }
  ...
  return (
    <>
      ...
      {user.role === "student" && page === "dashboard" && (
        <Dashboard user={user} complaints={complaints} setPage={setPage} />
      )}

      {user.role === "student" && page === "submit" && (
        <Submit user={user} complaints={complaints} setComplaints={setComplaints} />
      )}

      {user.role === "student" && page === "track" && (
        <Track complaints={complaints} />
      )}

      {user.role === "student" && page === "profile" && (
        <Profile user={user} />
      )}

      {user.role === "authority" && (
        <AuthDash user={user} complaints={complaints} setComplaints={setComplaints} />
      )}
    </>
  );
}
